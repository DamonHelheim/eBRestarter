...
public sealed partial class UC_RestartTask : UserControl
{
    ...
    /// <c>DataContext</c>. <see cref="ViewModelRestartTask"/> is registered as a singleton, so
    /// resolving it here yields exactly the same instance the hosting control supplies via
    /// <c>DataContext</c> — the binding source is unchanged, only the binding mechanism.
    /// </remarks>
